...
""", new Dictionary<string, object?>
    ...
    {
        var table=history?"T_RRPARTS__COM_PRECO_PRODUTO":"T_RRPARTS__COM_PRECO_PRODUTO_ATUAL"; if(!c.HasTable(table))return;
    ...
    private static async Task CopyApplications(BuilderContext c,CancellationToken ct) => await CopyRows(c,"T_RRPARTS__PCI_APLICACAO_RESUMO",ct,async(r,o,tx)=>await c.ExecuteAsync("INSERT INTO produto_aplicacoes(produto_id,descricao,marca,modelo,motor,ano_inicial,ano_final,raw_json) VALUES(@p,@d,@ma,@mo,@mt,@ai,@af,@r)",new Dictionary<string,object?>{{"@p",BuilderContext.DbValue(BuilderContext.Text(r,o,"COMPROD_ID"))},{"@d",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_APLICACAO","APLICACAO"))},{"@ma",BuilderContext.DbValue(BuilderContext.Text(r,o,"MARCA_VEICULO","DESC_MARCA"))},{"@mo",BuilderContext.DbValue(BuilderContext.Text(r,o,"MODELO","DESC_MODELO"))},{"@mt",BuilderContext.DbValue(BuilderContext.Text(r,o,"MOTOR","DESC_MOTOR"))},{"@ai",BuilderContext.DbValue(BuilderContext.Text(r,o,"ANO_INICIAL"))},{"@af",BuilderContext.DbValue(BuilderContext.Text(r,o,"ANO_FINAL"))},{"@r",BuilderContext.RowJson(r)}},tx,ct));
        ...
        await using(var tx=(SqliteTransaction)await c.Target.BeginTransactionAsync(ct))
        {
            ...
            while(await r.ReadAsync(ct))
            {
                ...
                await c.ExecuteAsync("INSERT INTO estoque(produto_id,empresa_id,deposito_id,quantidade_fisica,quantidade_reservada,quantidade_disponivel,quantidade_transito,quantidade_compra,quantidade_venda,custo_medio,ultimo_custo,custo_reposicao,custo_financeiro,ultima_compra,ultima_venda,raw_json) VALUES(@p,@e,@d,@qf,@qr,@qd,@qt,@qc,@qv,@cm,@uc,@cr,@cf,@co,@ve,@r)",new Dictionary<string,object?>
                {
                    ...
                    ["@qf"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"QTDE_ESTOQUE_FISICO","QTDE_ESTOQUE","QTDE_SALDO")),["@qr"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"QTDE_RESERVADA","QTDE_RESERVADO")),
                    ...
                    ["@cm"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"VALR_CUSTO_MEDIO","VALR_CUSTO_HISTORICO")),["@uc"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"VALR_ULTIMO_CUSTO")),
                    ...
                    ["@co"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"DATA_ULTIMA_COMPRA")),["@ve"]=BuilderContext.DbValue(BuilderContext.Text(r,o,"DATA_ULTIMA_VENDA")),["@r"]=BuilderContext.RowJson(r)
                },tx,ct);result.Written++;
            }
            await tx.CommitAsync(ct);
        }
        await CopyLocations(c,ct);return result;
    }

    private static async Task CopyLocations(BuilderContext c,CancellationToken ct)
    {
        const string table="T_RRPARTS__PCI_LOCACAO";if(!c.HasTable(table))return;
        ...
        while(await r.ReadAsync(ct))await c.ExecuteAsync("INSERT INTO localizacoes(produto_id,deposito_id,rua,prateleira,coluna,nivel,posicao,raw_json) VALUES(@p,@d,@rua,@prat,@col,@niv,@pos,@raw)",new Dictionary<string,object?>{{"@p",BuilderContext.DbValue(BuilderContext.Text(r,o,"PCIPMAT_ID"))},{"@d",BuilderContext.DbValue(BuilderContext.Text(r,o,"PCIDEPO_ID"))},{"@rua",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_RUA","RUA"))},{"@prat",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_PRATELEIRA","PRATELEIRA"))},{"@col",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_COLUNA","COLUNA"))},{"@niv",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_NIVEL","NIVEL"))},{"@pos",BuilderContext.DbValue(BuilderContext.Text(r,o,"DESC_POSICAO","POSICAO","CODG_LOCACAO"))},{"@raw",BuilderContext.RowJson(r)}},tx,ct);
        await tx.CommitAsync(ct);
    }
}
...
public sealed class FornecedorBuilder : IBuilderModule
{
    ...
    public async Task<ModuleResult> ExecuteAsync(BuilderContext c, CancellationToken ct)
    {
        var table = c.HasTable("T_RRPARTS__PAR_FORN_ENTIDADE")
            ? "T_RRPARTS__PAR_FORN_ENTIDADE"
            : "T_RRPARTS__PAR_FORNECEDOR";

        var result = new ModuleResult();
        if (!c.HasTable(table)) return result;

        await c.LogAsync("FORNECEDORES", $"Carregando clientes em memória para enriquecer {table}...");

        var clientes = new Dictionary<string, ClienteFornecedorResumo>(StringComparer.OrdinalIgnoreCase);
        await using (var cmd = c.Target.CreateCommand())
        {
            cmd.CommandText = "SELECT id,nome,fantasia,cpf_cnpj,email,telefone FROM clientes";
            cmd.CommandTimeout = 30;
            await using var cr = await cmd.ExecuteReaderAsync(ct);
            while (await cr.ReadAsync(ct))
            {
                var id = cr.IsDBNull(0) ? null : cr.GetString(0);
                if (string.IsNullOrWhiteSpace(id)) continue;
                clientes[id] = new ClienteFornecedorResumo(
                    cr.IsDBNull(1) ? null : cr.GetString(1),
                    cr.IsDBNull(2) ? null : cr.GetString(2),
                    cr.IsDBNull(3) ? null : cr.GetString(3),
                    cr.IsDBNull(4) ? null : cr.GetString(4),
                    cr.IsDBNull(5) ? null : cr.GetString(5));
            }
        }

        await c.LogAsync("FORNECEDORES", $"{clientes.Count:N0} clientes carregados. Lendo fornecedores...");
        ...
        while (await r.ReadAsync(ct))
        {
            ...
            var id = BuilderContext.Text(r, o, "PARENTD_ID", "PARFORN_ID", "ID");
            if (id is null)
            {
                result.Review++;
                await c.ReviewAsync(Name, table, "", "Sem identificador de fornecedor");
                continue;
            }

            clientes.TryGetValue(id, out var cliente);

            var nome = BuilderContext.Text(r, o, "NOME_RAZAO_SOCIAL", "NOME_FORNECEDOR", "NOME")
                       ?? cliente?.Nome;
            var fantasia = BuilderContext.Text(r, o, "NOME_FANTASIA")
                           ?? cliente?.Fantasia;
            var documento = BuilderContext.Text(r, o, "NUMR_CGC", "CNPJ", "CPF")
                            ?? cliente?.CpfCnpj;
            var email = BuilderContext.Text(r, o, "NOME_EMAIL", "EMAIL")
                        ?? cliente?.Email;
            var telefone = BuilderContext.Text(r, o, "NUMR_TELEFONE", "TELEFONE", "FONE")
                           ?? cliente?.Telefone;

            await c.ExecuteAsync("""
INSERT OR REPLACE INTO fornecedores
(id,nome,fantasia,cpf_cnpj,email,telefone,ativo,raw_json)
VALUES(@id,@nome,@fantasia,@doc,@email,@fone,@ativo,@raw)
""", new Dictionary<string, object?>
            {
                ["@id"] = id,
                ["@nome"] = BuilderContext.DbValue(nome),
                ["@fantasia"] = BuilderContext.DbValue(fantasia),
                ["@doc"] = BuilderContext.DbValue(documento),
                ["@email"] = BuilderContext.DbValue(email),
                ["@fone"] = BuilderContext.DbValue(telefone),
            ...
            if (result.Read % 100 == 0)
            {
                Console.WriteLine($"  Fornecedores: {result.Read:N0} lidos / {result.Written:N0} gravados");
                await c.LogAsync("FORNECEDORES_PROGRESS", $"{result.Read:N0} lidos; {result.Written:N0} gravados");
            }
        }

        await tx.CommitAsync(ct);
        await c.LogAsync("FORNECEDORES", $"Concluído: {result.Read:N0} lidos; {result.Written:N0} gravados; {result.Review:N0} revisão");
        return result;
    }

    private sealed record ClienteFornecedorResumo(
        string? Nome,
        string? Fantasia,
        string? CpfCnpj,
        string? Email,
