...
public sealed class DomainConsolidationService
{
        ...
        CancellationToken ct = default)
    {
            ...
            foreach (var root in roots)
            {
                await foreach (var source in ReadRowsAsync(connection, root, ct))
                {
                    ...
                    foreach (var relation in FindRelations(root, relationships))
                    {
                        ...
                        if (!source.Data.TryGetValue(rootColumn, out var value) || value is null || string.IsNullOrWhiteSpace(value.ToString())) continue;

                        var records = await ReadRelatedAsync(connection, relatedTable, relatedColumn, value.ToString()!, 250, ct);
                        if (records.Count > 0) related[relatedTable.OriginalName] = records;
                        if (records.Count >= 250)
                            review.Add(new ReviewRecord(domain, root.OriginalName, source.RowNumber, "RELATED_LIMIT", $"{relatedTable.OriginalName} atingiu o limite de registros relacionados"));
                    }
                    ...
                    var document = new
                    {
                        ...
                        {
                            valid = true,
                            warnings = related.Count == 0 ? new[] { "Nenhuma relação segura encontrada" } : Array.Empty<string>()
                        }
                    };
        ...
        await WriteReviewAsync(Path.Combine(consolidatedDirectory, "review.csv"), review);
    ...
    private static List<TableProfile> SelectRoots(IReadOnlyList<TableProfile> tables, IReadOnlyList<RelationshipCandidate> relationships)
    {
        ...
        var ranked = tables.Select(t => new
        {
            Table = t,
        ...
        var roots = ranked.Where(x => x.Score > 10).Take(12).Select(x => x.Table).ToList();
        if (roots.Count == 0 && ranked.Length > 0) roots.Add(ranked[0].Table);
        return roots;
    }
    ...
    {
        var candidates = table.Columns.Where(c => c.IsPrimaryKeyCandidate).Select(c => c.Name)
            .Concat(["ID", "PARENTD_ID", "COMPROD_ID", "PCIPROD_ID", "CODG_ENTIDADE", "CODG_PRODUTO", "NUMR_PEDIDO", "NUMR_NOTA", "CODIGO"]);
        foreach (var candidate in candidates.Distinct(StringComparer.OrdinalIgnoreCase))
            if (row.TryGetValue(candidate, out var value) && value is not null && !string.IsNullOrWhiteSpace(value.ToString()))
    ...
    {
        "CLIENTES-ENTIDADES" => "entity", "PRODUTOS-CATALOGO" => "product", "ESTOQUE-LOGISTICA" => "stock",
        "COMPRAS-SUPRIMENTOS" => "purchase", "VENDAS-COMERCIAL" => "sale", "FINANCEIRO" => "financial",
        "FISCAL-TRIBUTARIO" => "fiscal", "CONTABILIDADE" => "accounting", "GARANTIAS-DEVOLUCOES" => "warranty",
        _ => "generic"
    };
    ...
    private static async Task WriteReviewAsync(string path, IEnumerable<ReviewRecord> records)
    {
